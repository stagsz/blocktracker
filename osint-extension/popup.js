...
},
{
  ...
  "truncationPriority": [
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // AI Action Buttons - Quick open with AI providers
      document.getElementById('ai-claude-btn').addEventListener('click', () => {
        const prompt = document.getElementById('generated-prompt-text').value;
        openAIProvider('claude', prompt);
      });

      document.getElementById('ai-chatgpt-btn').addEventListener('click', () => {
        const prompt = document.getElementById('generated-prompt-text').value;
        openAIProvider('chatgpt', prompt);
      });

      document.getElementById('ai-perplexity-btn').addEventListener('click', () => {
        const prompt = document.getElementById('generated-prompt-text').value;
        openAIProvider('perplexity', prompt);
      });

      document.getElementById('ai-gemini-btn').addEventListener('click', () => {
        const prompt = document.getElementById('generated-prompt-text').value;
        openAIProvider('gemini', prompt);
      });

      // Helper function to open AI provider
      function openAIProvider(provider, prompt) {
        ...
        chrome.runtime.sendMessage({
          ...
          if (response && response.success) {
            showNotification(response.message || `Opening ${provider}...`);
          } else {
            showNotification('Failed to open AI provider');
          }
        });
      }
